...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=20):
        ...
        # define input & recurrent weights for the forget gates
    ...
    def forward(self, x, h=None, c=None):
        ...
        # loop over the time step & update parameters
        for xt in x.unbind(0):
            # get the gates and upate
            hiddens = self.W_hh(h) + self.W_xh(xt)
            gates, update = hiddens.split((3 * self.hidden_size, self.hidden_size), dim=-1)
