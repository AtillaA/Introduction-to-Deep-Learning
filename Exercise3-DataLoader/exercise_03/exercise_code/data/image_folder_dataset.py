...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = self.labels[index]  # label of the image to retrieve
        path = self.images[index]   # name of the image to retrieve
        image = self.load_image_as_numpy(path)

        if self.transform is not None: # apply transformation if not empty
